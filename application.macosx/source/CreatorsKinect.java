...
public void setupConfigScreen() {
  ...
  cp5.addSlider2D("gridSizeSelector")
    ...
    .setArrayValue(new float[] {PApplet.parseInt(settings.get("grid-cols")), PApplet.parseInt(settings.get("grid-rows"))})
  ...
  cp5.addSlider("updatePeriod")
    ...
    .setWidth(400)
    .setRange(0.1f,10)
    .setValue(PApplet.parseInt(settings.get("feed-update-period")) / 1000.f)    
  ...
  cp5.addRange("kinectRange")
    ...
    .setWidth(400)
    .setRange(1000,10000)
    .setRangeValues(PApplet.parseInt(settings.get("kinect-near")), PApplet.parseInt(settings.get("kinect-far")))    
  ...
  cp5.addRange("kinectSensitivity")
    ...
    .setWidth(400)
    .setRange(0,1000)
    .setRangeValues(PApplet.parseInt(PApplet.parseFloat(settings.get("kinect-min-force")) * 1000), PApplet.parseInt(PApplet.parseFloat(settings.get("kinect-max-force")) * 1000))    
    .setDecimalPrecision(0);
...
public void controlEvent(ControlEvent theEvent) {
  ...
  if(theEvent.getName().equals("gridSizeSelector")) {
    ...
    Range r = (Range)theEvent.getController();
    float min = r.getArrayValue()[0] / 1000.f;
    float max = r.getArrayValue()[1] / 1000.f;    
...
public class KinectManager {
  ...
  PImage depthMap;
  int depthThreshold = PApplet.parseInt(settings.get("kinect-far"));
...
class PhotoArranger {
  ...
  public int gridRows = PApplet.parseInt(settings.get("grid-rows"));  // 9
  public int gridCols = PApplet.parseInt(settings.get("grid-cols")); // 15
  ...
  float HAND_MIN_DISTANCE = PApplet.parseFloat(settings.get("kinect-near"));
  float HAND_MAX_DISTANCE = PApplet.parseFloat(settings.get("kinect-far"));
  float HAND_MIN_FORCE    = PApplet.parseFloat(settings.get("kinect-min-force"));  // 0.2
  float HAND_MAX_FORCE    = PApplet.parseFloat(settings.get("kinect-max-force"));  // 0.9
  ...
  public PhotoArranger(CreatorsKinect applet) {
    ...
    physics = new VerletPhysics2D();
    physics.setDrag(PApplet.parseFloat(settings.get("physics-drag")));
    //physics.setWorldBounds(new Rect(0,0, width, height));
...
class PhotoLoader implements Runnable {
  ...
  int UPDATE_FREQUENCY = PApplet.parseInt(settings.get("feed-update-period"));  // milliseconds
...
public float tweenEaseInOutBack(float step, float duration, float min, float max, float overshoot) {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#000000", "--hide-stop", "CreatorsKinect" });
  }
}
